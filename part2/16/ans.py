...
# 3. метод _b() скрыт в родительском классе А и "публичен" в его потомке B.
#       => Имеется ввиду, что у нас есть доступ к методу _b() в классе B. Сам метод _b() не публичен, ни в A, ни в B!
# 4. метод _b() скрыт в родительском классе А и скрыт в его потомке B => извне не подразумевается вызов _b().

# Интересная ситуация с методом __c() - от доступен только внутри класса A.
